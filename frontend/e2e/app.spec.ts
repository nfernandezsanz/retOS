...
async function visualAuditCoverage(page: Page) {
  return await page.evaluate(
    ({ sections, modules }) => {
      const sectionLinks = [...document.querySelectorAll(".sidebar nav a")].map(
        (link) => link.textContent?.trim(),
      );
      return {
        sections,
        modules,
        visible_sections: sectionLinks.filter((label): label is string =>
          Boolean(label),
        ),
        tooltip_targets: [
...
async function mockProviderApi(page: Page) {
  ...
  const jobs = [
    jobFixture("job-seed-1", "ingest.source", "succeeded"),
    failedIndexJob,
  ];
    ...
    cases: [
      {
        ...
        question:
          "What did Apollo guidance computers use for mission operations?",
        passed: true,
    ...
    cases: [
      {
        ...
        question:
          "Compare HotpotQA supporting facts for Vela and United States Air Force.",
      },
    ],
  };
  ...
  function evalRunsForRequestUrl(url: string) {
    ...
    return domainId
      ? evalRuns.filter((run) => run.job.domain_id === domainId)
      : evalRuns;
  }
    ...
    return Array.from(grouped.entries()).map(([suiteName, runs]) => {
      ...
      const metricNames = Array.from(
        new Set(
          runs.flatMap((run) =>
            Object.keys(
              (run.report?.metrics as Record<string, number> | undefined) ?? {},
            ),
          ),
        ),
      ).sort();
      return {
        ...
        pass_rate:
          runs.filter((run) => run.report?.passed === true).length /
          runs.length,
        latest: {
  ...
  function recordDomainGrantAudit(
    adminUserId: string,
    eventType: string,
    domainId: string,
  ) {
  ...
  await page.route(
    /http:\/\/localhost:8000\/admin\/users\/[^/]+\/domain-grants(?:\/[^/]+)?$/,
    async (route) => {
      ...
      if (route.request().method() === "POST") {
        ...
        domainGrants[adminUserId] = [
          ...domainGrants[adminUserId].filter(
            (item) => item.domain_id !== payload.domain_id,
          ),
          grant,
        ];
        recordDomainGrantAudit(
          adminUserId,
          "admin_user.domain_grant_created",
          payload.domain_id,
        );
        await route.fulfill({
          contentType: "application/json",
      ...
      if (route.request().method() === "DELETE" && domainId) {
        domainGrants[adminUserId] = domainGrants[adminUserId].filter(
          (item) => item.domain_id !== domainId,
        );
        recordDomainGrantAudit(
          adminUserId,
          "admin_user.domain_grant_deleted",
          domainId,
        );
  ...
  await page.route(
    /http:\/\/localhost:8000\/admin\/users\/[^/]+\/status/,
    async (route) => {
  ...
  await page.route(
    /http:\/\/localhost:8000\/admin\/users\/[^/]+\/password/,
    async (route) => {
      const adminUserId = new URL(route.request().url()).pathname.split("/")[3];
      const target = adminUsers.find((user) => user.id === adminUserId);
      if (!target) {
    ...
    await route.fulfill({
      ...
      json: {
        ...
        env: {
          ...
          RETOS_MODEL:
            payload.provider === "local" ? "ollama:gemma4" : "gpt-5-mini",
        ...
        missing_config: [],
        warnings: [
          "Restart API and worker after changing runtime environment.",
        ],
        reason: null,
      },
    });
  ...
  await page.route(
    ...
    async (route) => {
      ...
      await route.fulfill({
        contentType: "application/json",
        json: domains.filter(
          (domain) => includeArchived || !domain.archived_at,
        ),
      });
    },
  );
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+(?:\/restore)?$/,
    async (route) => {
            ...
            changes: [
              {
                field: "archived_at",
                before: null,
                after: domains[index].archived_at,
              },
            ],
          },
        });
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/documents/,
    async (route) => {
      ...
      await route.fulfill({
        ...
        json: documents.filter(
          (document) =>
            document.domain_id === domainId &&
            (includeArchived || !document.archived_at),
        ),
      });
    },
  );
  await page.route(
    /http:\/\/localhost:8000\/documents\/[^/]+\/restore$/,
    async (route) => {
  ...
  await page.route(
    /http:\/\/localhost:8000\/documents\/[^/]+\/history$/,
    async (route) => {
      const documentId = new URL(route.request().url()).pathname.split("/")[2];
              ...
              changes:
                event.event_type === "document.updated"
                  ? [
                      {
                        field: "title",
                        before: "Uploaded Fixture",
                        after: "Uploaded Fixture Reviewed",
                      },
                    ]
  ...
  await page.route(
    /http:\/\/localhost:8000\/documents\/[^/]+\/versions$/,
    async (route) => {
      const documentId = new URL(route.request().url()).pathname.split("/")[2];
      const document = documents.find((item) => item.id === documentId);
      if (!document) {
      ...
      await route.fulfill({
        contentType: "application/json",
        json: documentVersions.filter(
          (version) => version.document_id === documentId,
        ),
      });
    },
  );
  await page.route(
    /http:\/\/localhost:8000\/document-versions\/[^/]+\/artifacts$/,
    async (route) => {
      const versionId = new URL(route.request().url()).pathname.split("/")[2];
      await route.fulfill({
        contentType: "application/json",
        json: artifacts.filter(
          (artifact) => artifact.document_version_id === versionId,
        ),
      });
    },
  );
  await page.route(
    /http:\/\/localhost:8000\/document-versions\/[^/]+\/segments$/,
    async (route) => {
      const versionId = new URL(route.request().url()).pathname.split("/")[2];
      await route.fulfill({
        contentType: "application/json",
        json: segments.filter(
          (segment) => segment.document_version_id === versionId,
        ),
      });
    },
  );
  ...
  await page.route(
    /http:\/\/localhost:8000\/jobs\?limit=\d+/,
    async (route) => {
      await route.fulfill({
        contentType: "application/json",
        json: jobs,
  ...
  await page.route(
    /http:\/\/localhost:8000\/jobs\/[^/]+\/retry$/,
    async (route) => {
      const jobId = new URL(route.request().url()).pathname.split("/")[2];
  ...
  await page.route(
    /http:\/\/localhost:8000\/audit\/journal-events\?limit=\d+/,
    async (route) => {
      await route.fulfill({
        contentType: "application/json",
        json: journalEvents,
  ...
  await page.route(
    /http:\/\/localhost:8000\/audit\/progress-events\?limit=\d+/,
    async (route) => {
      await route.fulfill({
        contentType: "application/json",
        json: progressEvents,
  ...
  await page.route(
    /http:\/\/localhost:8000\/audit\/export\?limit=\d+/,
    async (route) => {
      await route.fulfill({
        contentType: "application/json",
        headers: {
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/sources/,
    async (route) => {
      const pathParts = new URL(route.request().url()).pathname.split("/");
      const domainId = pathParts[2];
  ...
  await page.route(
    /http:\/\/localhost:8000\/sources\/[^/]+\/scan/,
    async (route) => {
      const job = jobFixture("job-scan-1", "ingest.source", "queued", {
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/index\/rebuild/,
    async (route) => {
      const job = jobFixture("job-index-1", "index.domain", "queued", {
        requested_at: "now",
      });
      jobs.unshift(job);
      recordAudit(job);
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/ingestions\/text/,
    async (route) => {
      const domainId = new URL(route.request().url()).pathname.split("/")[2];
      documents.push({
        id: "document-2",
      ...
      const job = jobFixture("job-text-1", "ingest.source", "queued", {
        title: "Policy Note",
      });
      jobs.unshift(job);
      recordAudit(job);
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/ingestions\/upload/,
    async (route) => {
      const domainId = new URL(route.request().url()).pathname.split("/")[2];
  ...
  await page.route(
    /http:\/\/localhost:8000\/domains\/[^/]+\/queries/,
    async (route) => {
      await route.fulfill({
        contentType: "application/json",
        json: {
              ...
              steps: [
                {
                  name: "search",
                  description:
                    "Run bounded BM25 search over the selected domain.",
                  status: "planned",
                },
                {
                  name: "read",
                  description:
                    "Read only citations returned by controlled corpus search.",
                  status: "planned",
                },
              ],
            },
  ...
  await page.route(
    "http://localhost:8000/evals/agent-multihop",
    async (route) => {
      const job = jobFixture(
  ...
  await page.route("http://localhost:8000/evals/squad", async (route) => {
    ...
    const domainId =
      typeof payload.domain_id === "string" ? payload.domain_id : null;
    const reportPaths = {
  ...
  await page.route(
    "http://localhost:8000/evals/hotpotqa-agent",
    async (route) => {
      const payload = route.request().postDataJSON() as Record<string, unknown>;
      expect(payload.domain_id).toBe("domain-123");
      const domainId =
  ...
  await page.route(
    "http://localhost:8000/evals/natural-questions",
    async (route) => {
      const payload = route.request().postDataJSON() as Record<string, unknown>;
      const domainId =
        typeof payload.domain_id === "string" ? payload.domain_id : null;
  ...
  await page.route(
    "http://localhost:8000/evals/ocr-benchmark",
    async (route) => {
      const payload = route.request().postDataJSON() as {
        dataset_path: string;
  ...
  await page.route(
    /http:\/\/localhost:8000\/evals\/runs\?.*/,
    async (route) => {
  ...
  await page.route(
    /http:\/\/localhost:8000\/evals\/runs\/trends\?.*/,
    async (route) => {
      const runs = evalRunsForRequestUrl(route.request().url());
      await route.fulfill({
        contentType: "application/json",
  ...
  await page.route(
    /http:\/\/localhost:8000\/evals\/runs\/([^/]+)\/rerun/,
    async (route) => {
      const match = route
        .request()
        .url()
        .match(/\/evals\/runs\/([^/]+)\/rerun$/);
  ...
  await page.route(
    /http:\/\/localhost:8000\/evals\/runs\/compare.*/,
    async (route) => {
      expect(new URL(route.request().url()).searchParams.get("domain_id")).toBe(
        "domain-123",
      );
      await route.fulfill({
        contentType: "application/json",
  ...
  await page.route(
    /http:\/\/localhost:8000\/evals\/runs\/regression-gate.*/,
    async (route) => {
      expect(new URL(route.request().url()).searchParams.get("domain_id")).toBe(
        "domain-123",
      );
...
test("keeps the RetOS brand system accessible and responsive", async ({
  page,
}) => {
  ...
  await expect(page.locator(".brand-brief")).toContainText(
  ...
  await expect(
    page.getByLabel("System metrics").getByText("Build"),
  ).toBeVisible();
  await expect(
    page.getByLabel("System metrics").getByText("2026.06.29-local"),
  ).toBeVisible();
  ...
  const theme = await page.evaluate(() => {
    ...
    const primaryAction = getComputedStyle(
      document.querySelector(".primary-action") as HTMLElement,
    );
    const panel = getComputedStyle(
      document.querySelector(".panel") as HTMLElement,
    );
    return {
...
test("keeps operational modules compact and segmented", async ({ page }) => {
  ...
  for (const [section, module] of modules) {
        ...
        return {
          ...
          tooltipTargets: [
            ...document.querySelectorAll(
              ".workspace [data-tooltip], .sidebar [data-tooltip]",
            ),
          ].filter((element) => element.getClientRects().length > 0).length,
    ...
    expect(
      layout.bodyHorizontalOverflow,
      `${section}/${module ?? "overview"} horizontal overflow`,
    ).toBe(false);
    expect(
      layout.workspaceScroll,
      `${section}/${module ?? "overview"} workspace scroll`,
    ).toBeLessThanOrEqual(0);
    expect(
      layout.activeHeight,
      `${section}/${module ?? "overview"} active height`,
    ).toBeLessThanOrEqual(650);
    expect(
      layout.panelScroll,
      `${section}/${module ?? "overview"} panel scroll`,
    ).toBeLessThanOrEqual(
      section === "documents" && module === "documents-library" ? 140 : 20,
    );
    expect(
      layout.activeScroll,
      `${section}/${module ?? "overview"} active scroll`,
    ).toBeLessThanOrEqual(
      section === "documents" && module === "documents-library" ? 140 : 20,
    );
    expect(
      layout.tooltipTargets,
      `${section}/${module ?? "overview"} tooltip targets`,
    ).toBeGreaterThan(0);
  }
  ...
  const mediumDocumentLayout = await page
    .locator("#documents")
    .evaluate((panel) => {
...
test("seeds the local demo corpus from the overview", async ({ page }) => {
  ...
  await expect(
    page.locator("#documents").getByText("retos-demo", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Domain documents").getByText("Apollo Guidance Notes"),
  ).toBeVisible();
  await expect(
    page
      .getByRole("status")
      .getByText("Demo corpus ready: 2 created, 0 skipped."),
  ).toBeVisible();
});

test("clears expired stored admin sessions", async ({ page }) => {
  await page.evaluate(() =>
    localStorage.setItem("retos.adminToken", "expired-token"),
  );
  await page.route("http://localhost:8000/llm/providers", async (route) => {
  ...
  const storedToken = await page.evaluate(() =>
    localStorage.getItem("retos.adminToken"),
  );
  expect(storedToken).toBeNull();
});

test("clears expired stored admin sessions from workspace refresh", async ({
  page,
}) => {
  await page.evaluate(() =>
    localStorage.setItem("retos.adminToken", "expired-token"),
...
test("loads the operational console", async ({ page }) => {
  ...
  await expect(
    primaryNavigation.getByRole("link", { name: "Documents" }),
  ).toBeVisible();
  await expect(
    primaryNavigation.getByRole("link", { name: "Documents" }),
  ).toHaveAttribute("data-tooltip", /Manage domains/);
  await expect(
    page.getByRole("link", { name: "Grounded query workflow" }),
  ).toBeVisible();

  await page.getByRole("link", { name: "Admin" }).first().click();
  await expect(
    page.locator("#admin").getByRole("heading", { name: "LLM providers" }),
  ).toBeVisible();
  const workspaceContext = page.getByLabel("Current workspace context");
  await expect(
    workspaceContext.getByText("Admin", { exact: true }),
  ).toBeVisible();
  await expect(
    workspaceContext.getByText("Providers", { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Admin modules").getByRole("link", { name: "Providers" }),
  ).toHaveAttribute("data-tooltip", /provider readiness/);
  await page.locator(".module-nav a[href='#admin-providers']").hover();
  await expect(page.getByRole("tooltip")).toContainText(
    "Load local and paid LLM provider readiness",
  );
  await expect(page.getByRole("tooltip")).toHaveCSS("position", "fixed");
  await expect(
    page.getByLabel("Admin modules").getByRole("link", { name: "Users" }),
  ...
  await page
    ...
    .click();
  await expect(
    workspaceContext.getByText("Users", { exact: true }),
  ).toBeVisible();
  ...
  await page
    ...
    .fill("retos-dev-admin-change-me");
  await page.getByRole("button", { name: "Load providers" }).click();
  ...
  await expect(
    page
      .getByLabel("Provider runtime context")
      .getByText("Ollama", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Provider runtime context").getByText("deepagents"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Provider runtime context")
  ...
  await expect(
    page.locator(".provider-summary").getByText("Agent runtime"),
  ).toBeVisible();
  await expect(page.getByText("ollama:gemma4")).toBeVisible();
  await expect(
    page.locator(".provider-summary").getByText("deepagents"),
  ).toBeVisible();
  await expect(
    page.locator(".provider-summary").getByText("Paid providers blocked"),
  ).toBeVisible();
  ...
  await page
    .getByLabel("Runtime plan agent runtime")
    .selectOption("deepagents");
  await page.getByRole("button", { name: "Preview plan" }).click();
  await expect(
    page
      .getByLabel("Runtime environment plan")
      .getByText("Can start after restart"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Runtime environment plan")
      .getByText("RETOS_AGENT_RUNTIME"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Runtime environment plan").getByText("deepagents"),
  ).toBeVisible();
  ...
  await expect(
    openAiProviderRow.getByText("Missing RETOS_OPENAI_API_KEY"),
  ).toBeVisible();
  await page
    .getByLabel("Admin modules")
    .getByRole("link", { name: "Users" })
    .click();
  await expect(
    page.getByLabel("Admin users context").getByText("Active users"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Admin users").getByText("admin@retos.dev"),
  ).toBeVisible();
  await expect(
    page
  ...
  await expect(
    uiAdminRow.getByText("ui-admin@retos.dev", { exact: true }),
  ).toBeVisible();
  ...
  await uiAdminRow
    .getByLabel("Role for ui-admin@retos.dev")
    .selectOption("admin");
  await uiAdminRow.getByRole("button", { name: "Update role" }).click();
  await expect(uiAdminRow.getByText("admin", { exact: true })).toBeVisible();
  await expect(
    page.getByText("Updated role for ui-admin@retos.dev"),
  ).toBeVisible();
  await uiAdminRow
    .getByLabel("Grant domain to ui-admin@retos.dev")
    .selectOption("domain-123");
  await uiAdminRow.getByRole("button", { name: "Grant" }).click();
  await expect(
    uiAdminRow.locator(".grant-chip").getByText("smoke-research"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Updated password for ui-admin@retos.dev"),
  ).toBeVisible();
  ...
  await expect(
    page
      .locator("#documents")
      .getByRole("heading", { name: "Domains and documents" }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Documents modules").getByRole("link", { name: "Library" }),
  ).toHaveAttribute("href", "#documents-library");
  await expect(
    page.getByLabel("Document library context").getByText("Smoke Research"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Document library context").getByText("Active only"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Create domain")
      .locator("summary")
  ...
  await expect(
    page
      .getByLabel("Document library context")
      .locator("[data-tooltip]")
      .filter({
  ...
  await expect(
    page.getByLabel("Domain documents").getByText("Corpus"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Domain documents").getByText("Documents", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Domain documents").getByText("1 visible"),
  ).toHaveAttribute("data-tooltip", /selected domain/);
  const documentLibraryLayout = await page
    .locator("#documents")
    .evaluate((panel) => {
  ...
  await page
    .getByLabel("Documents modules")
    .getByRole("link", { name: "Sources" })
    .click();
  await expect(
    page.getByLabel("Document sources context").getByText("Smoke Research"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Document sources context").getByText("Local rebuild"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Document sources context")
      .locator("[data-tooltip]")
      .filter({
  ...
  await expect(
    page
      .getByLabel("Domain sources")
      .locator("summary")
      .filter({ hasText: "Registered sources" }),
  ...
  await expect(
    page
      .getByLabel("Domain sources")
      .getByText("Corpus inputs", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Domain sources")
      .locator(".source-list-heading")
      .getByText("Registered sources"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Domain sources").getByText("1 registered"),
  ).toHaveAttribute("data-tooltip", /active domain/);
  await expect(
    page.getByLabel("Domain sources").getByText("Mounted Corpus"),
  ).toBeVisible();
  await page
    .getByLabel("Documents modules")
  ...
  await expect(
    page.getByLabel("Evidence for Smoke Document").getByText("v1"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Artifacts for Smoke Document").getByText("raw_text"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Artifacts for Smoke Document")
      .getByText("memory://smoke-doc#raw-text"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Segments for Smoke Document").getByText("page=1"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Segments for Smoke Document")
      .getByText("Smoke segment text for search readiness."),
  ).toBeVisible();
  await page
    .getByLabel("Documents modules")
  ...
  await expect(
    page.getByLabel("Upload ingestion context").getByText("Smoke Research"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Upload ingestion context")
      .locator("[data-tooltip]")
      .filter({
  ...
  await expect(
    page.getByLabel("Queries modules").getByRole("link", { name: "Live" }),
  ).toHaveAttribute("data-tooltip", /SSE progress/);
  await page
    .getByLabel("Queries modules")
    .getByRole("link", { name: "Live" })
    .click();
  await expect(
    page.getByRole("button", { name: "Connect live updates" }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Live progress context").getByText("SSE stream"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Live progress summary").getByText("Waiting"),
  ).toBeVisible();
  await page
    .getByLabel("Queries modules")
    ...
    .click();
  await expect(
    page.getByLabel("Query run context").getByText("Smoke Research"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query run context").getByText("Local Deep Agents"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Query result status").getByText("Result status"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Grounded answer").getByRole("heading", { name: "Answer" }),
  ).toBeVisible();
  await expect(page.getByLabel("Query evidence review")).toBeVisible();
  await expect(
    page.getByLabel("Grounded answer").locator("[data-tooltip]").filter({
      hasText: "Evidence linked",
    }),
  ).toHaveAttribute("data-tooltip", /linked back/);
  await expect(
    page.getByLabel("Query budget usage").getByText("Within budget"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query budget usage").getByText("Evidence linked 1/1"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query budget usage").getByText("Contradictions 0"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query budget usage").getByText("Citations 1/5"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query budget usage").getByText("Route single segment"),
  ).toBeVisible();
  await page.getByText("Query plan", { exact: true }).click();
  await expect(
    page.getByLabel("Query plan").getByText("multi hop evidence route"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query plan").getByText("expects multi document"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Query plan")
      .getByText("evidence mentions readiness search"),
  ).toBeVisible();
  await page.getByText("Evidence route", { exact: true }).click();
  await expect(
    page.getByLabel("Evidence route").getByText("single citation"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Evidence route").getByText("Smoke Document"),
  ).toBeVisible();
  await page.getByText("Multi-hop audit", { exact: true }).click();
  await expect(
    page.getByLabel("Multi-hop audit").getByText("supported multi document"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Multi-hop audit").getByText("multi-hop question"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Multi-hop audit")
      .getByText("Bridge terms: search, readiness"),
  ).toBeVisible();
  await page.getByText("Citations", { exact: true }).click();
  await expect(
    page.getByLabel("Query citations").getByText("Smoke Document"),
  ).toBeVisible();
  await page.getByText("Neighbor context", { exact: true }).click();
  await expect(
    page.getByLabel("Neighbor context").getByText("Adjacent context"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Query citations").getByText("page=1"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Live progress summary").getByText("agent.started"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Live progress summary").getByText("2 live events"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Live progress summary").getByText("1 queued"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Live progress events")
      .getByText("Persisted progress replayed"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Live progress events").getByText("Agent query started"),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByLabel("Active domain")
      .getByRole("option", { name: "Policy Review" }),
  ).toBeAttached();
  ...
  await expect(
    page.getByLabel("Domain sources").getByText("Policy Corpus"),
  ).toBeVisible();
  await page.getByRole("button", { name: "Edit Policy Corpus" }).click();
  await page
    .getByLabel("Source name for Policy Corpus")
    .fill("Policy Corpus Reviewed");
  await page
    .getByLabel("Source URI for Policy Corpus")
    .fill("file:///corpus/policy-reviewed");
  await page.getByRole("button", { name: "Save Policy Corpus" }).click();
  await expect(
    page.getByLabel("Domain sources").getByText("Policy Corpus Reviewed"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Domain sources")
      .getByText("file:///corpus/policy-reviewed"),
  ).toBeVisible();
  ...
  page.once("dialog", async (dialog) => {
    expect(dialog.message()).toContain(
      'Remove source "Policy Corpus Reviewed"',
    );
    await dialog.accept();
  });
  await page
    .getByRole("button", { name: "Remove Policy Corpus Reviewed" })
    .click();
  await expect(
    page.getByLabel("Domain sources").getByText("Policy Corpus Reviewed"),
  ).toBeHidden();
  ...
  await expect(
    page.getByRole("button", { name: "Save domain" }),
  ).toBeDisabled();
  ...
  await page
    .getByPlaceholder("Uploaded research note")
  ...
  await page
    .getByPlaceholder("Uploaded research note")
    .fill("Uploaded Fixture");
  await page.getByRole("button", { name: "Queue upload" }).click();
  await expect(
    page.getByLabel("Domain documents").getByText("Uploaded Fixture"),
  ).toBeVisible();
  await page.getByRole("button", { name: "Edit Uploaded Fixture" }).click();
  await page
    .getByLabel("Document title for Uploaded Fixture")
    .fill("Uploaded Fixture Reviewed");
  await page.getByRole("button", { name: "Save Uploaded Fixture" }).click();
  await expect(
    page.getByLabel("Domain documents").getByText("Uploaded Fixture Reviewed"),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Archive Uploaded Fixture Reviewed" })
    .click();
  await expect(
    page.getByLabel("Domain documents").getByText("Uploaded Fixture Reviewed"),
  ).toBeHidden();
  ...
  await page
    .getByRole("button", { name: "Restore Uploaded Fixture Reviewed" })
    .click();
  await expect(
    page.getByRole("button", { name: "Archive Uploaded Fixture Reviewed" }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "History Uploaded Fixture Reviewed" })
    .click();
  await expect(
    page
      .getByLabel("History for Uploaded Fixture Reviewed")
      .getByText("document.updated"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("History for Uploaded Fixture Reviewed")
      .getByText("Uploaded Fixture Reviewed"),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "History Uploaded Fixture Reviewed" })
    .click();
  ...
  await page
    ...
    .click();
  await expect(
    page.getByLabel("Pasted text context").getByText("Policy Review"),
  ).toBeVisible();
  await expect(
  ...
  await page
    .getByPlaceholder("Research note", { exact: true })
    .fill("Policy Note");
  await page
    .getByPlaceholder(
      "Paste local fixture text, notes, transcripts, or extracted content.",
    )
    .fill(
      "A policy note that can be ingested without touching paid providers.",
    );
  await page.getByRole("button", { name: "Queue text ingestion" }).click();
  await expect(
    page.getByLabel("Domain documents").getByText("Policy Note"),
  ).toBeVisible();
  ...
  await expect(
    page
      .locator("#audit")
      .getByRole("heading", { name: "Jobs and evidence ledger" }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit modules").getByRole("link", { name: "Events" }),
  ).toHaveAttribute("href", "#audit-events");
  await expect(
    page.getByLabel("Audit jobs context").getByText("Visible jobs"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Recent jobs").getByText("job-text-1"),
  ).toBeVisible();
  await expect(page.getByText("title: Policy Note")).toBeVisible();
  await page.getByRole("button", { name: "Refresh audit" }).click();
  await page
    .getByLabel("Audit modules")
    .getByRole("link", { name: "Events" })
    .click();
  await expect(
    page.getByLabel("Audit events context").getByText("Journal rows"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit events context").getByText("Hash-chain evidence"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Journal events").getByText("job.created").first(),
  ).toBeVisible();
  await expect(
    page.getByLabel("Journal events").getByText("Hash 2222222222222222"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Journal events").getByText("document.archived"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Journal events").getByText("document.restored"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Journal events").getByText("job-text-1"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Persisted progress events")
      .getByText("Queued ingest.source")
      .first(),
  ).toBeVisible();
  await page
    .getByLabel("Audit modules")
    ...
    .click();
  await expect(
    page.getByLabel("Audit progress context").getByText("Grouped jobs"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit progress context").getByText("Progress events"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    scanProgressGroup.getByText("Completed ingest.source"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Audit export integrity").getByText("Export integrity"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit export integrity").getByText("valid"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Audit export integrity")
      .getByText("retos.audit-export.v2"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Audit export integrity")
      .getByText("make audit-export-check"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit export integrity").getByText("Continuity gaps"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Audit export integrity").getByText("0").first(),
  ).toBeVisible();

  await page.getByRole("link", { name: "Evals" }).first().click();
  await expect(
    page.getByLabel("Evals modules").getByRole("link", { name: "History" }),
  ).toHaveAttribute("data-tooltip", /regression-gate/);
  await expect(
    page.getByLabel("Active eval scope").getByText("Dataset reports"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Eval results context").getByText("retos-smoke"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Eval metrics").getByText("retrieval recall"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval metadata").getByText("built-in"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval metadata").getByText("retos-smoke-fixtures"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("apollo-guidance"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval history context").getByText("Comparable runs"),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("Eval run history").getByText("retos-smoke"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval run history").getByText("3 cases"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval trends").getByText("retos-smoke"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval trends").getByText("retrieval recall"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Eval metrics").getByText("multi hop support"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval metadata").getByText("agent-multihop-fixtures"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("apollo-telemetry-bridge"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("invoice-retention-policy"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("incident-escalation-triage"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("agent-multihop"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval trends").getByText("agent-multihop"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Active eval scope").getByText("Smoke Research"),
  ).toHaveCount(2);
  ...
  await expect(
    page.getByLabel("Eval metadata").getByText("squad-v2"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval metadata").getByText("/evals/datasets/ui-squad.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("squad-mars-red-planet"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-squad.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-squad.md"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("squad-v2"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Eval run history").getByText("2 cases"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Eval cases").getByText("hotpotqa-vela-air-force"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-hotpotqa.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-hotpotqa.md"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("hotpotqa"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Eval cases").getByText("hotpotqa-agent-vela-air-force"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval metadata").getByText("hotpotqa-agent"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-hotpotqa-agent.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-hotpotqa-agent.md"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("hotpotqa-agent"),
  ).toBeVisible();
  ...
  await page
    .locator("summary")
    .filter({ hasText: "Natural Questions" })
    .click();
  ...
  await page
    .getByLabel("Natural Questions report stem")
    .fill("ui-natural-questions");
  await page
    .getByRole("button", { name: "Run Natural Questions eval" })
    .click();
  await expect(
    page.getByLabel("Eval cases").getByText("natural-questions-123"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-natural-questions.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-natural-questions.md"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("natural-questions"),
  ).toBeVisible();
  ...
  await page
    .getByLabel("OCR benchmark path")
    .fill("ocr-benchmark/manifest.json");
  ...
  await expect(
    page.getByLabel("Eval metrics").getByText("character error rate"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval cases").getByText("receipt-001"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-ocr-benchmark.json"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval report paths").getByText("ui-ocr-benchmark.md"),
  ).toBeVisible();
  await page
    .getByLabel("Evals modules")
    .getByRole("link", { name: "History" })
    .click();
  await expect(
    page.getByLabel("Eval run history").getByText("ocr-manifest"),
  ).toBeVisible();
  ...
  await expect(
    page.getByLabel("Eval trends").getByText("2 runs"),
  ).toBeVisible();

  await page.getByRole("button", { name: "Compare latest" }).click();
  await expect(
    page.getByLabel("Eval comparison").getByText("retos-smoke"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval comparison").getByText("squad-v2"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval comparison").getByText("retrieval recall"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval comparison").getByText("unchanged"),
  ).toBeVisible();

  await page.getByRole("button", { name: "Regression gate" }).click();
  await expect(
    page.getByLabel("Eval regression gate").getByText("Promote"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval regression gate").getByText("0 metric regressions"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval regression gate").getByText("retrieval recall"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Eval regression gate").getByText("2%"),
  ).toBeVisible();

  await page.getByRole("link", { name: "Audit" }).first().click();
  await page.getByLabel("Filter jobs").selectOption("index.domain");
  await expect(
    page.getByLabel("Recent jobs").getByText("job-index-1"),
  ).toBeVisible();
  await page
    ...
    .click();
  await expect(
    page.getByLabel("Selected job detail").getByText("job-index-1"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Selected job detail")
      .getByText("index.domain", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Selected job detail").getByText("requested_at"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Selected job detail").getByText("Queued index.domain"),
  ).toBeVisible();
  await page
    .getByLabel("Recent jobs")
    .locator("article")
  ...
  await expect(
    page
      .getByLabel("Selected job detail")
      .getByText("job-retry-job-failed-index-1"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Selected job detail").getByText("retried_from_job_id"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Selected job detail").locator("pre"),
  ).toContainText('"retried_from_job_id": "job-failed-index-1"');
...
test("keeps provider controls usable on mobile", async ({ page }) => {
  ...
  await expect(
    page
      .getByLabel("Available LLM providers")
      .getByText("Ollama local runtime"),
  ).toBeVisible();
  await page.getByRole("link", { name: "Evals" }).first().click();
  await expect(
    page.locator("#evals").getByRole("heading", { name: "Local evals" }),
  ).toBeVisible();
  await page.getByRole("link", { name: "Audit" }).first().click();
  await expect(
    page
