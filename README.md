...
prints the password when it is still the obvious local placeholder outside production;
custom `.env` passwords stay hidden.
...
custom bootstrap admin password or a production-mode placeholder.
...
| Query with citations | Run grounded agent queries against indexed domains, then review separated result status, bounded answer text, budget chips, citations, query plans, and evidence accordions |
...
| Local access | `make local-access` | Prints local URLs plus the bootstrap admin email and only the non-production development placeholder password; custom `.env` passwords and production-mode placeholders are acknowledged without being echoed. |
...
| Process tracker | `make process-tracker-check` | Validates the phase tracker structure, local evidence, coverage numbers, audit hooks, and residual promotion blockers. |
...
| Audit handoff manifest | `make audit-manifest OUTPUT=evals/reports/audit-manifest.json` | Exports commit state, generation context, coverage evidence from `backend/coverage.json`, required gates, critical file hashes including the auditor evidence matrix and branding assets, visual audit coverage/screenshot records, and remaining external promotion evidence as JSON. |
...
| Audit handoff report | `make audit-handoff-report MANIFEST=evals/reports/audit-manifest.json OUTPUT=evals/reports/audit-handoff.md` | Exports a human-readable Markdown companion with candidate, verdict, evidence-status table, coverage source, local gates, blockers, promotion decision checklist, hashes, visual coverage, and visual evidence. |
...
| Calibration evidence | `make eval-calibration-evidence OUTPUT=docs/releases/evidence/<release>-calibration.md` | Converts the ignored calibration manifest into path-safe Markdown evidence with metrics, gates, dataset profile/suite provenance, source URLs, records, and license notes. |
...
| Calibration trend gate | `make eval-calibration-trend-gate` | Validates versioned trend evidence offline: PASS status, baseline/candidate sample growth, required targets, dataset profile/suite provenance, HTTPS source URLs, license notes, metric-level regression tolerance, and no local dataset/report path leakage. |
...
| Visual audit evidence gate | `make visual-audit-check` | Validates the local visual-audit manifest offline, including expected desktop/mobile screenshot records, PNG existence, byte sizes, SHA-256 hashes, viewport dimensions, section/module coverage, tooltip coverage, and responsive no-overflow checks. |
