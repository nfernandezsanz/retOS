...
| Backend coverage | 92.72% total coverage; 85.26% branch-only coverage is ratcheted in CI while the project works toward the 90% branch target. |
...
| Backend tests | `make test` | Runs pytest with the 90% total coverage gate and an explicit branch coverage ratchet from `coverage.json` (`BRANCH_COVERAGE_MIN`, currently 85.26%, target 90%). |
