...
| Backend coverage | 92.26% total coverage; 84.16% branch-only coverage is ratcheted in CI while the project works toward the 90% branch target. |
...
| Backend tests | `make test` | Runs pytest with the 90% total coverage gate and an explicit branch coverage ratchet from `coverage.json` (`BRANCH_COVERAGE_MIN`, currently 84.16%, target 90%). |
