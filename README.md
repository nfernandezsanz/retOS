...
| Audit handoff manifest | `make audit-manifest OUTPUT=evals/reports/audit-manifest.json` | Exports commit/CI status, required gates, critical file hashes, visual audit screenshot records, and remaining external promotion evidence as JSON. |
...
| Visual audit screenshots | `make frontend-visual-audit` | Generates ignored desktop and mobile PNG snapshots under `frontend/visual-audit/`; CI uploads the same PNGs as a `retos-visual-audit-<commit>` artifact. |
...
| Current HEAD CI | `make ci-status-check` | Queries GitHub Actions and verifies the current commit has successful backend, frontend, docker, and final audit-evidence jobs. |
