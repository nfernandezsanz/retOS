...
| Backend coverage | 94.17% total coverage; 87.96% branch-only coverage is ratcheted in CI while the project works toward the 90% branch target. |
...
| Backend tests | `make test` | Runs pytest with the 90% total coverage gate and an explicit branch coverage ratchet from `coverage.json` (`BRANCH_COVERAGE_MIN`, currently 87.96%, target 90%). |
