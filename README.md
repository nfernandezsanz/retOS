...
returns indexed evidence plus journal/progress hash-chain events and a valid limited
audit export.
...
| Search or demo flow feels broken | `make local-smoke` | Exercise login, idempotent demo seeding, indexed search, API readiness/version, journal/progress hash-chain evidence, limited audit export integrity, and the web console against the running stack. |
...
| Local smoke | `make local-smoke` | Hits the already-running local web/API stack, authenticates with the bootstrap admin, re-seeds demo data idempotently, and verifies demo search plus journal/progress hash-chain and limited audit export evidence. |
