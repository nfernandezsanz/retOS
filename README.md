...
| --- | --- |
| Navigate the console | Use the compact Overview, then switch to Documents, Queries, Evals, Audit, or Admin; each long screen has module pills for its main task areas |
...
- A React + TypeScript + Vite frontend scaffold focused on operational visibility for
  ...
  sections, plus contextual module pills that shorten Documents, Queries, Evals, Audit,
  and Admin into scan-friendly task areas. The UI includes hover/focus tooltips,
...
| Browser smoke | `make frontend-e2e` | Opens the React console with Playwright and verifies section navigation, tooltip contract, contextual module pills, visible UI state, admin user management, document edit/archive/restore/history, agent multi-hop and dataset-backed evals, eval rerun, eval comparison, and eval trend flows. |
