...
| --- | --- | --- | --- |
| Friendly document and domain management UI | React console covers demo corpus seeding, domain creation/selection, source inventory, document upload, text ingestion, edit/archive/restore/history, evidence inspection, and job progress. | `make frontend-e2e` | Local evidence ready; human visual review still required before production promotion. |
...
| 90% or better test coverage | Backend total coverage is 95.35%; branch coverage is ratcheted at 90.65%. Tests and eval smoke avoid paid providers. | `make check` | Local evidence ready; coverage numbers must be refreshed when the suite changes. |
...
| Evals and calibration | Deterministic eval smoke, agent multi-hop evals, dataset-backed adapters, path-safe 200-record/40-case calibration evidence, and path-safe 100-record/30-case to 200-record/40-case trend evidence with release metric gates are documented and checked offline. | `make eval-smoke`, `make eval-agent-multihop`, `make eval-calibration-gate`, and `make eval-calibration-trend-gate` | Pilot-ready evidence; broader public calibration remains a promotion decision. |
...
| Release and production handoff | Release workflow requests GHCR publish, SBOM/provenance, Cosign signing/verification, visual audit artifacts, audit manifest artifacts, a machine-checked backup/restore drill template, and a machine-checked human promotion evidence template. | `make release-check`, `make production-preflight`, `make backup-restore-drill-check`, and `make promotion-template-check` | Not production-promoted until tag publish, real digests, SBOM/provenance, Cosign evidence, and target-environment review are recorded. |
