...
| 90% or better test coverage | Backend total coverage is 95.37%; branch coverage is 90.68% with the minimum ratcheted at 90.65%. Tests and eval smoke avoid paid providers. | `make check` | Local evidence ready; coverage numbers must be refreshed when the suite changes. |
