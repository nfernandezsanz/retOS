...
| Switchable local and paid LLM providers | Provider catalog exposes local Ollama `gemma4`, OpenAI, Anthropic, Google, OpenRouter, Azure, and safe missing-configuration hints. Paid calls require explicit opt-in. | `make check` and `make api-smoke` | Local-safe by default; target deployment must review provider keys and cost policy. |
...
| 90% or better test coverage | Backend total coverage is 95.22%; branch coverage is ratcheted at 90.45%. Tests and eval smoke avoid paid providers. | `make check` | Local evidence ready; coverage numbers must be refreshed when the suite changes. |
