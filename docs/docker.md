...
The smoke test uses a temporary Compose project, builds the app images, runs migrations, starts Postgres, RabbitMQ, API, worker, and web, waits for healthchecks, hits health, auth, domain/source CRUD, job creation/listing, SSE, and web over HTTP, then removes its temporary containers and volumes.
