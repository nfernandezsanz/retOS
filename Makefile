...
help:
	...
	@printf "  make target-security-review-check Validate target security review evidence template\n"
	@printf "  make visual-review-check Validate visual review evidence template\n"
	...
	@printf "  make process-tracker-check Validate phase tracker coverage, gates, and blockers\n"
	@printf "  make auditor-evidence-matrix-check Validate objective-to-evidence traceability\n"
	@printf "  make readme-check     Validate README onboarding, pills, status, and agent workflow\n"
	...
	@printf "  make calibration-scope-decision-check Validate calibration scope decision template\n"
...
local-smoke:
	"$(BACKEND_PYTHON)" scripts/local_smoke.py

local-logs:
	docker compose logs --no-color --tail 160 postgres rabbitmq migrate api worker web
...
target-security-review-check:
	$(PYTHON) scripts/check_target_security_review.py $(if $(TEMPLATE),--template "$(abspath $(TEMPLATE))",)

visual-review-check:
	$(PYTHON) scripts/check_visual_review.py $(if $(TEMPLATE),--template "$(abspath $(TEMPLATE))",)
...
process-tracker-check:
	$(PYTHON) scripts/check_process_tracker.py $(if $(TRACKER),--tracker "$(abspath $(TRACKER))",)

auditor-evidence-matrix-check:
	scripts/check_auditor_evidence_matrix.sh

readme-check:
	$(PYTHON) scripts/check_readme_usability.py

auditor-static-check: dependency-audit security-policy-check target-security-review-check visual-review-check env-security-check ignore-hygiene-check operations-runbook-check backup-restore-drill-check promotion-template-check process-tracker-check auditor-evidence-matrix-check readme-check brand-check visual-audit-check ci-workflow-check release-workflow-check release-notes-check versioned-release-notes-check eval-calibration-gate eval-calibration-trend-gate calibration-scope-decision-check release-check production-preflight audit-pack-check audit-manifest-check audit-handoff-report-check audit-bundle-check
...
calibration-scope-decision-check:
	$(PYTHON) scripts/check_calibration_scope_decision.py $(if $(TEMPLATE),--template "$(abspath $(TEMPLATE))",)
