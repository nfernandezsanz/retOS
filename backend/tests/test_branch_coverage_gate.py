...
def test_branch_coverage_gate_compares_displayed_rounded_percentage(
    ...
    report_path = write_coverage_report(
        tmp_path / "coverage.json",
        covered_branches=1143,
        num_branches=1370,
    )

    exit_code = gate.run(coverage_json=report_path, fail_under=83.43)

    assert exit_code == 0
    assert "Backend branch coverage: 83.43% (1143/1370 branches)" in capsys.readouterr().out
