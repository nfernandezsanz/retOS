...
async def create_admin_user(
    ...
    async with uow:
        ...
        await uow.journal_events.add(
            ...
            entity_id=admin.id,
            payload={"email": admin.email, "is_active": admin.is_active},
        )
        try:
            await uow.commit()
...
async def update_admin_user_status(
    ...
    async with uow:
        ...
        active_count = await uow.admin_users.count_active()
        if admin.is_active and not payload.is_active and active_count <= 1:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
