...
| 4 - Product UI | In progress | Operational console renders domain creation/selection, document inventory, live progress ledger, pipeline, query execution with citations, audit placeholder, and admin provider catalog states | Browser smoke covers provider login/catalog, domain creation, document inventory, live progress SSE parsing, and agent query result flow with mocked API | Frontend type/build gate active | Provider readiness, cost guardrails, query job status, selected domain, document evidence, progress events, and citations visible in UI | API integration guide includes frontend runtime notes | Source management, uploads, full document CRUD, and persisted SSE resume semantics still pending |
