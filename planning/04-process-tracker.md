...
| 4 - Product UI | In progress | Operational console renders domain creation/selection, document inventory, pipeline, query execution with citations, audit placeholder, and admin provider catalog states | Browser smoke covers provider login/catalog, domain creation, document inventory, and agent query result flow with mocked API | Frontend type/build gate active | Provider readiness, cost guardrails, query job status, selected domain, document evidence, and citations visible in UI | API integration guide includes frontend runtime notes | Streaming UX, source management, uploads, and full document CRUD screens still pending |
