...
| 4 - Product UI | In progress | Operational console renders domain creation/selection, document/source inventory, text ingestion, scan/index queueing, live progress ledger, pipeline, query execution with citations, audit placeholder, and admin provider catalog states | Browser smoke covers provider login/catalog, domain creation, document/source inventory, text ingestion, scan/index queueing, live progress SSE parsing, and agent query result flow with mocked API | Frontend type/build gate active | Provider readiness, cost guardrails, query job status, selected domain, source/document evidence, queued/recent jobs, progress events, and citations visible in UI | API integration guide includes frontend runtime notes | File uploads, full document CRUD, richer job grouping, and persisted SSE resume semantics still pending |
